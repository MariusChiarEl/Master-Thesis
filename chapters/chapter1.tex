...
The Blockchain \cite{blockchainAttacks} is a technology used in various applications, such as cryptocurrencies, Internet of Things, electronic voting and many others. Its transparent, fully distributed, peer to peer and append-only nature is its most notable strength. The transactions, which may have a different meaning depending on the application they are used for, are publicly visible and cannot be modified after they are published. This means the users may verify any transaction with no need for a centralized authority.

Bitcoin, the most popular cryptocurrency at this moment, takes advantage of these characteristics, creating a decentralized financial system. Unfortunately, there are also significant shortcomings in terms of security for this kind of networks. One of the most notorious instances where these vulnerabilities had been exploited is known as ``The DAO'' \cite{blockchainAttacks}, where in 2016 an unknown attacker managed to drain \$50 million USD through a \textit{Reentrancy Attack}.
...
According to table 1.1, the static analysis tools, primarly represented by Slither, enjoy a higher popularity than the symbolic execution tools, represented by Manticore. This fact is shown by the number of stars on GitHub, which indicate how popular a tool is. These tools share the principle of scanning the code without executing it, but do so in different ways.
...
contract FoobarContract {
    function foobar(int a, int b) public pure returns (int) {
        int x = 1;
        int y = 0;
        ...
        require(x - y != 0, "Assertion failed: x - y == 0");
...
\end{BVerbatim}
\caption{Example of code to analyze. Adapted from Roberto B et al. \cite{symbolicExecutionDef}}
\end{figure}
