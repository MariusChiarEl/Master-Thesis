...
As shown in the diagram above, the usual workflow in which SmartScan is involved starts with the developer writing the smart contract code and pushing it in the GitHub repository. Our application intervenes right after, when the developer copies the repository link in it and starts the analysis. Afterwards, the application returns a security report, based on which the developer starts fixing the vulnerabilities and the cycle may repeat. Whenever the application runs the analysis, it makes sure to fetch the latest version of the project so the developer doesn't need to.
...
\multicolumn{1}{|c|}{Project \#1 \cite{sampleProject1}} & 3/5 $\star$             & \multicolumn{1}{c|}{16}  & \multicolumn{1}{c|}{10}     & \multicolumn{1}{c|}{3}    & \multicolumn{1}{c|}{0}        & 29    \\
\multicolumn{1}{|c|}{Project \#2 \cite{sampleProject2}} & 4/5 $\star$             & \multicolumn{1}{c|}{44}  & \multicolumn{1}{c|}{9}      & \multicolumn{1}{c|}{9}    & \multicolumn{1}{c|}{0}        & 62    \\
...
This table shows the results SmartScan obtained by analyzing three similar smart contracts. The first project has the best results, having only three high severity vulnerabilities and a three star rating. The second one shows the highest amount of errors, caused primarily by the 44 low severity errors, but did not surpass the five star threshold of 10 high severity errors, so it earned four out of five stars. At the same time, while it has the least total errors, the third project has the most high severity errors, which grant it the five star rating.
...
\hline
Project            & Time   \\ \hline
Sample Project \#2 \cite{sampleProject2} & 6.56 s \\
Sample Project \#3 \cite{sampleProject3} & 8.65 s \\
Sample Project \#1 \cite{sampleProject1} & 9.16 s \\ \hline
\end{tabular}
\caption{Time needed for cloning and analyzing the three projects.}
\label{tab:my-table}
\end{table}

On the subject of SmartScan's speed, it will mostly be determined by Slither's high performance in terms of analysis and REST API's speed of cloning a project, which may be limited by the user's internet speed, especially when approaching complex smart contracts. Considering this application's aim to help students and begginer developers, we will stick to measuring the cloning and analysis speed of the same three projects from the previous table. For these measurements we used the internal stopwatch and took the average of three runs for each project to avoid anomalies. We can conclude that, for small projects, SmartScan can provide a reasonably quick analysis because all of these projects took less than 10 seconds to analize accross all the runs. The execution time of this project is especially important for a teacher who needs to evaluate their students' projects or assessments and needs to run the same process on tens of repositories.

Another important topic regarding SmartScan is its ability to guide a begginer user through the analysis and audit process. The Code Area of the application is used to host messages that direct the user's attention to each step of the routine and provides explainations as to how to access and interpret the scanned code. This feature helps the user have a streamlined experience, paired with the relatively low run time for the analysis itself. Additionally, SmartScan uses a simple and familiar design, with the Code Area as its central component, which shares its space with the File Tree. The user may resize the two elements as needed or hide the File Tree completely to better focus on the code. Another user-friendly feature of SmartScan is showing various messages in case of failure (for instance, if the user enters an invalid repository link).

SmartScan achieves its goals to ease the audit process and guide its users to easily track the detected errors and optimizations by automizing most of the work, compiling an overall security report and highlighting the affected sections of the code while also naturally integrating into the development and testing process. Its qualities make it suitable in an academic setting, used by the students to see various coding patterns that lead to security vulnerabilities or inefficient usage of gas, or in a team of begginer developers who wish to provide the most secure solutions for their users. However, there are also multiple ways to improve the application and the user's experience. Firstly, a very useful feature would be generating a separate file that organizes the errors and helps the developer track the errors even more easily and to which they can add other errors, found through manual analysis or with the help of other tools. Additionally, Smart Scan could integrate the functions to review the differences, commit and push the improved code back to the repository to make the process as seamless as possible. Another improvement to this application can take the form of a separate window that shows the last clicked error so they can easily see the errors one by one. Needless to say, SmartScan is still a prototype, but it's able to achieve its goals at a basic level and has a clear path forward in terms of improvements.
...
%[Future work / Good to have additions: highlight different severity vulnerabilities with different colors (gradient from yellow to red), use git.diff, separate window that shows the error in the affected code under the cursor, making it a web application or a VS Code extension, allow analyzing multiple projects and creating more cloned repos to review]